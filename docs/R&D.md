...
5. Interop Layer - let's you run legacy architecture in new architecture - You'll still get warning to migrate to proper new architecture

6. RN automation

7. New architecture only API's in css introduced in 0.79

8. IOS pre builds

9. Element nodes - https://reactnative.dev/docs/element-nodes - 0.82

10. Appearence.setColorScheme doesn't accept null/undefined instead use unspecified

11. Measure App start up time

12. React Native voice alternative library or custom module integration

13. LtpText - POC using EventBus
...
14. React Native splash screen in wave vs boot splash

15. Watchlist swipe card with Flashlist custom component

16. Re-visit on Tab focused for Carousal tabs - For new UI the static naming with hook isn't going to be performant

17. React native c++ library for file logging
...
3. 16 kb page support we need to do and edge to edge support in android 16. Bundle compression , Precompiled Ios built config.
...
7. Keyboard controller -- Control each frame of the keyboard movement in react native. reference: https://kirillzyusko.github.io/react-native-keyboard-controller/?s=08
...
1. This way, it's easy to replace the hook with other React Libraries like React
4. Query, GraphQL etc.. in the future
...
8. use isInitialRenderRef to control expensive utils computations on component rerenders
...
1. Brotli compression - HOLD
2. React native bundle visualizer
3. Package upgradation
1. Back button handler fix on Android 16
2. Predictive back behaviour fix
4. let's work on cleaning up the un-used files, assets using the scripts in the project.
5.   Clean up console log
6.   Run prettier-write
7.   Keep adding new console logs - Progressive
