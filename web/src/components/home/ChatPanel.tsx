...
export const ChatPanel = ({
  ...
  return (
    <section className="p-3 flex flex-col overflow-hidden bg-white" style={{ width: `${widthPercent}%` }}>
      <div className="flex flex-col gap-1 mb-2 shrink-0">
        ...
        {selectedSectionTitle && (
          <div className="text-[10px] text-slate-700 bg-purple-50 border border-purple-300 px-2 py-1 rounded-md flex items-center gap-1">
            ...
            <span className="font-semibold text-purple-700">{selectedSectionTitle}</span>
            {onClearSelectedSection && (
              <button
      ...
      >
        {chatMessages.length === 0 && <p className="text-xs text-slate-600">Start a conversation about the selected document.</p>}
        {chatMessages.map((msg, index) => (
          <div
            key={`${msg.role}-${msg.createdAt}-${index}`}
            className={`px-3 py-2 rounded-lg shadow-sm ${
              msg.role === "user" ? "bg-purple-600 text-white ml-8" : "bg-white border border-slate-200 mr-2 text-slate-900"
            }`}
          >
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className={`text-[9px] ${msg.role === "user" ? "text-purple-100" : "text-slate-500"}`}>
                {new Date(msg.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              </span>
            </div>
            {msg.role === "assistant" ? (
              <div className="prose prose-slate prose-xs max-w-none text-xs prose-p:my-4 prose-p:leading-relaxed prose-p:text-slate-900 prose-ul:my-4 prose-ul:pl-6 prose-ul:space-y-2 prose-ol:my-4 prose-ol:pl-6 prose-ol:space-y-2 prose-li:leading-relaxed prose-li:pl-2 prose-li:text-slate-900 prose-headings:mt-6 prose-headings:mb-3 prose-headings:font-bold prose-headings:text-xs prose-headings:text-slate-900 prose-h1:text-xs prose-h2:text-xs prose-h3:text-xs prose-pre:my-4 prose-pre:bg-slate-50 prose-pre:border prose-pre:border-slate-300 prose-pre:text-xs prose-pre:rounded-lg prose-code:bg-purple-100 prose-code:text-purple-900 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-xs prose-code:font-mono prose-code:font-medium prose-strong:font-semibold prose-strong:text-slate-900 prose-a:text-purple-600 prose-a:underline prose-a:hover:text-purple-700">
                <ReactMarkdown
          ...
          <div className="px-3 py-2 rounded-lg bg-white border border-slate-200 shadow-sm mr-2">
            <div className="flex items-center justify-between">
              ...
              {streamingCharCount > 0 && (
                <span className="text-[10px] font-mono text-purple-600">{streamingCharCount} chars</span>
              )}
            </div>
          </div>
        )}
      ...
      <form className="flex flex-col gap-2" onSubmit={onSend}>
        <textarea
          ...
          disabled={!isDocSelected || isStreaming}
          rows={2}
        />
        <div className="flex gap-2">
          <button
            ...
            disabled={!isDocSelected || isStreaming || !chatInput.trim()}
          >
